...
let phoneUsers = filteredUsers.map(users => users.phone);
let totalFilterBalance = filteredUsers.reduce((acc, user) => acc + parseFloat(user.balance.replace('$','').replace(',','')),0);
let totalBalance = users.reduce((acc, user) => acc + parseFloat(user.balance.replace('$','').replace(',','')),0);
console.log(phoneUsers);
console.log(totalFilterBalance.toFixed(2));
